const searchCandies = (searchString, maxPrice) => {
  let filteredCandies = candies.filter(candy => candy.price <= maxPrice && candy.name.toLocaleLowerCase().indexOf(searchString.toLocaleLowerCase()) === 0)
  filteredCandies = filteredCandies.map(candy => candy.name)
